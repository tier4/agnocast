...
static long agnocast_ioctl(struct file * file, unsigned int cmd, unsigned long arg)
{
  ...
  if (cmd == AGNOCAST_GET_VERSION_CMD) {
    ...
    if (args.topic_name.len >= TOPIC_NAME_BUFFER_SIZE) goto return_EINVAL;
    char * topic_name_buf = kmalloc(args.topic_name.len + 1, GFP_KERNEL);
    if (!topic_name_buf) goto return_ENOMEM;
    if (copy_from_user(topic_name_buf, (char __user *)args.topic_name.ptr, args.topic_name.len)) {
      kfree(topic_name_buf);
      goto return_EFAULT;
    }
    topic_name_buf[args.topic_name.len] = '\0';
