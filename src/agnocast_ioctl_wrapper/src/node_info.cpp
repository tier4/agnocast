...
char ** get_agnocast_sub_topics(const char * node_name, int * topic_count)
{
  ...
  *topic_count = 2;
  ...
  char ** topic_array = (char **)malloc(*topic_count * sizeof(char *));
  if (!topic_array) {
    return nullptr;
  }

  for (size_t i = 0; i < *topic_count; i++) {
    topic_array[i] = (char *)malloc((topics[i].size() + 1) * sizeof(char));
    if (!topic_array[i]) {
...
char ** get_agnocast_pub_topics(const char * node_name, int * topic_count)
{
  ...
  *topic_count = 2;
  ...
  char ** topic_array = (char **)malloc(*topic_count * sizeof(char *));  // NOLINT
  if (!topic_array) {
    return nullptr;
  }

  for (size_t i = 0; i < *topic_count; i++) {
    topic_array[i] = (char *)malloc((topics[i].size() + 1) * sizeof(char));  // NOLINT
    if (!topic_array[i]) {
      for (size_t j = 0; j < i; j++) {
