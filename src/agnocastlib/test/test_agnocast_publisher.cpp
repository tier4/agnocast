...
MOCK_GLOBAL_FUNC2(
  initialize_publisher, uint32_t(const uint32_t publisher_pid, const std::string & topic_name));
MOCK_GLOBAL_FUNC5(
...
// {
//   EXPECT_GLOBAL_CALL(
//     borrow_loaned_message_core, borrow_loaned_message_core(dummy_tn, _, _, _, _))
//     .WillOnce(testing::Return(std::vector<uint64_t>()));
//   EXPECT_GLOBAL_CALL(publish_core, publish_core(dummy_tn, _, _, _)).Times(1);

//   agnocast::ipc_shared_ptr<std_msgs::msg::Int32> message =
//   dummy_publisher->borrow_loaned_message(); dummy_publisher->publish(std::move(message));
