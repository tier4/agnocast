...
MOCK_GLOBAL_FUNC2(
  ...
  topic_local_id_t(const uint32_t publisher_pid, const std::string & topic_name));
MOCK_GLOBAL_FUNC5(
...
{
  EXPECT_GLOBAL_CALL(
    borrow_loaned_message_core, borrow_loaned_message_core(dummy_tn, id, dummy_qd, _, _))
    .WillOnce(testing::Return(std::vector<uint64_t>()));
  EXPECT_GLOBAL_CALL(publish_core, publish_core(dummy_tn, id, _, _)).Times(1);
...
{
  EXPECT_GLOBAL_CALL(borrow_loaned_message_core, borrow_loaned_message_core(dummy_tn, _, _, _, _))
    .WillOnce(testing::Return(std::vector<uint64_t>()));
  EXPECT_GLOBAL_CALL(publish_core, publish_core(dummy_tn, _, _, _)).Times(1);
...
TEST_F(AgnocastPublisherTest, test_publish_different_message)
{
  ...
  EXPECT_GLOBAL_CALL(
    borrow_loaned_message_core, borrow_loaned_message_core(diff_dummy_tn, id, 10, _, _))
    .WillOnce(testing::Return(std::vector<uint64_t>()));
