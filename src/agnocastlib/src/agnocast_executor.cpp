  ...
  AgnocastExecutables & agnocast_executables, const int timeout_ms) const
{
  ...
  for (uint32_t i = 0; i < receive_args.pub_shm_info.ret_publisher_num; i++) {
    const pid_t pid = receive_args.pub_shm_info.ret_publisher_pids[i];
    const uint64_t addr = receive_args.pub_shm_info.ret_shm_addrs[i];
    const uint64_t size = receive_args.pub_shm_info.ret_shm_sizes[i];
