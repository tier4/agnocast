...
void poll_for_unlink()
{
    ...
    struct ioctl_get_version_args kmod_version)
  {
    ...
    size_t copy_len = heaphook_version_str_len < (VERSION_BUFFER_LEN - 1)
                        ? heaphook_version_str_len
    ...
    if (!compare_to_minor_version(&lib_ver, &kmod_ver)) {
      RCLCPP_ERROR(
        ...
        kmod_ver.major, kmod_ver.minor, kmod_ver.patch, lib_ver.major, lib_ver.minor,
        lib_ver.patch);
      return false;
    }
