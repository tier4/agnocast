...
void NodeTimeSource::attachNode(agnocast::Node * node)
{
  ...
  // This ensures /clock callbacks are processed independently from other callbacks.
...
void NodeTimeSource::create_clock_sub()
{
  ...
  // This ensures /clock callbacks are processed even when main executor threads are busy
  clock_executor_ = std::make_unique<AgnocastOnlySingleThreadedExecutor>();
  clock_executor_->add_callback_group(clock_callback_group_);
