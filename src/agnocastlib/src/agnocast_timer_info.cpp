...
{
  // TODO: Implement with new TimerInfo structure
  std::lock_guard<std::mutex> lock(id2_timer_info_mtx);
  ...
  if (timerfd_settime(info.timer_fd, 0, &spec, nullptr) == -1) {
    throw std::runtime_error(
      "timerfd_settime failed in cancel: " + std::string(std::strerror(errno)));
  }
}
...
void reset_timer(uint32_t timer_id)
{
  ...
  if (timerfd_settime(info.timer_fd, 0, &spec, nullptr) == -1) {
    throw std::runtime_error(
      "timerfd_settime failed in reset: " + std::string(std::strerror(errno)));
  }

  const auto now = std::chrono::steady_clock::now();
  info.next_call_time_ns.store(to_nanoseconds(now) + period_count, std::memory_order_relaxed);
}
