...
bool BridgeIpcEventLoopP::spin_once(int timeout_ms)
{
...
void BridgeIpcEventLoopP::set_peer_mq_handler(EventCallback cb)
{
  peer_cb_ = std::move(cb);
}

void BridgeIpcEventLoopP::set_signal_handler(SignalCallback cb)
{
  signal_cb_ = std::move(cb);
}

void BridgeIpcEventLoopP::set_reload_handler(ReloadCallback cb)
{
  reload_cb_ = std::move(cb);
}

void BridgeIpcEventLoopP::ignore_signals(std::initializer_list<int> signals)
{
  struct sigaction sa
  {
  };
...
sigset_t BridgeIpcEventLoopP::block_signals(std::initializer_list<int> signals)
{
  sigset_t mask;
  sigemptyset(&mask);
  for (int sig : signals) {
...
void BridgeIpcEventLoopP::setup_mq()
{
  mq_self_name_ = create_mq_name_for_bridge();
  mq_unlink(mq_self_name_.c_str());
  mq_peer_fd_ = create_and_open_mq(mq_self_name_, "Performance Peer");
}

void BridgeIpcEventLoopP::setup_signals()
{
...
void BridgeIpcEventLoopP::setup_epoll()
{
  ...
  if (mq_peer_fd_ != -1) {
    add_fd_to_epoll(mq_peer_fd_, "Performance Peer MQ");
  }
...
mqd_t BridgeIpcEventLoopP::create_and_open_mq(const std::string & name, const std::string & label)
{
  struct mq_attr attr = {};
...
void BridgeIpcEventLoopP::add_fd_to_epoll(int fd, const std::string & label) const
{
...
void BridgeIpcEventLoopP::cleanup_resources()
{
  if (epoll_fd_ != -1) {
