...
CallbackIsolatedAgnocastExecutor::CallbackIsolatedAgnocastExecutor(const rclcpp::ExecutorOptions & options)
...
void CallbackIsolatedAgnocastExecutor::spin()
{
    ...
    std::vector<std::pair<rclcpp::CallbackGroup::SharedPtr, rclcpp::node_interfaces::NodeBaseInterface::SharedPtr>> groups_and_nodes;
    ...
    bool notify)
{
    ...
    // Confirm that group_ptr does not refer to any of the callback groups held by nodes in weak_nodes_.
    for (const auto & weak_node : weak_nodes_) {
        auto n = weak_node.lock();
        if (!n) continue;
        if (n->callback_group_in_node(group_ptr)) {
            RCLCPP_ERROR(logger, "Callback group already exists in node: %s", n->get_fully_qualified_name());
            close(agnocast_fd);
            exit(EXIT_FAILURE);
        }
    }
...
std::vector<rclcpp::CallbackGroup::WeakPtr>
CallbackIsolatedAgnocastExecutor::get_automatically_added_callback_groups_from_nodes_internal() const
{
    std::vector<rclcpp::CallbackGroup::WeakPtr> groups;
    ...
    for (const auto & weak_node : weak_nodes_) {
        ...
        if (!node) continue;
        node->for_each_callback_group(
            [&groups](rclcpp::CallbackGroup::SharedPtr group) {
                if (group && group->automatically_add_to_executor_with_node()) {
                    groups.push_back(group);
                }
            });
    ...
    rclcpp::node_interfaces::NodeBaseInterface::SharedPtr node_ptr, bool notify)
{
    ...
    // Confirm that any callback group in weak_groups_to_nodes_ does not refer to any of the callback groups held by node_ptr.
    for (const auto & weak_group_to_node : weak_groups_to_nodes_) {
        auto group = weak_group_to_node.first.lock();
        if (!group) continue;

        if (node_ptr->callback_group_in_node(group)) {
            RCLCPP_ERROR(logger, "One of the callback groups in node %s already exists in the executor.",
    ...
    if (!insert_info.second) {
        RCLCPP_ERROR(logger, "Node already exists in the executor: %s", node_ptr->get_fully_qualified_name());
        close(agnocast_fd);
        exit(EXIT_FAILURE);
    }
}
...
void CallbackIsolatedAgnocastExecutor::add_node(
    rclcpp::Node::SharedPtr node_ptr, bool notify)
{
    ...
    bool notify)
{
    ...
    if (it != weak_nodes_.end()) {
        ...
        RCLCPP_ERROR(logger, "Node not found in the executor: %s", node_ptr->get_fully_qualified_name());
        close(agnocast_fd);
        exit(EXIT_FAILURE);
    }
}
