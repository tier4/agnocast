...
extern std::mutex id2_subscription_info_mtx;
extern std::unordered_map<uint32_t, SubscriptionInfo> id2_subscription_info;
extern std::atomic<uint32_t> next_subscription_id;
...
uint32_t register_subscription(
  ...
  const bool is_transient_local, mqd_t mqdes, const rclcpp::CallbackGroup::SharedPtr callback_group)
{
  ...
  uint32_t subscription_id = next_subscription_id.fetch_add(1);

  {
    std::lock_guard<std::mutex> lock(id2_subscription_info_mtx);
    id2_subscription_info[subscription_id] =
      SubscriptionInfo{topic_name,     subscriber_id,   is_transient_local, mqdes,
...
void receive_message(
  [[maybe_unused]] uint32_t subscription_id,  // for CARET
