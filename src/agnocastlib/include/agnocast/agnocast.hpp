...
// Implementation of Node::create_subscription (deferred to avoid circular dependencies)
template <typename MessageT, typename Func>
typename Subscription<MessageT>::SharedPtr Node::create_subscription(
  const std::string & topic_name, size_t queue_size, Func && callback, SubscriptionOptions options)
{
  return std::make_shared<Subscription<MessageT>>(
