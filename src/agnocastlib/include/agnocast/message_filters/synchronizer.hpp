...
{
  using RealTypeCount =
    typename ::message_filters::mp_count<std::tuple<M0, M1, M2, M3, M4, M5, M6, M7, M8>, NullType>::type;
  using Messages = std::tuple<M0, M1, M2, M3, M4, M5, M6, M7, M8>;
  using Signal = Signal9<M0, M1, M2, M3, M4, M5, M6, M7, M8>;
  using Events = std::tuple<
    MessageEvent<M0 const>, MessageEvent<M1 const>, MessageEvent<M2 const>,
    MessageEvent<M3 const>, MessageEvent<M4 const>, MessageEvent<M5 const>,
    MessageEvent<M6 const>, MessageEvent<M7 const>, MessageEvent<M8 const>>;
