...
public:
  ...
  agnocast::ipc_shared_ptr<MessageT> take(bool allow_same_message = false)
  {
    ...
    for (uint32_t i = 0; i < take_args.pub_shm_info.ret_publisher_num; i++) {
      const pid_t pid = take_args.pub_shm_info.ret_publisher_pids[i];
      const uint64_t addr = take_args.pub_shm_info.ret_shm_addrs[i];
      const uint64_t size = take_args.pub_shm_info.ret_shm_sizes[i];
